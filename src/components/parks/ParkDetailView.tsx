...
import { getParkTopData } from "@/data/parkTopData";
import { TopAttractionsSection } from "@/components/parks/TopAttractionsSection";
...
export function ParkDetailView({ park }: { park: Park }) {
  const topData = useMemo(() => getParkTopData(park.slug), [park.slug]);
    ...
    <div className="min-h-full bg-offwhite">
      ...
      <section className="relative min-h-[52vh] overflow-hidden bg-navy sm:min-h-[58vh]">
        ...
        <div className="relative z-10 mx-auto flex h-full min-h-[52vh] max-w-7xl flex-col justify-end px-4 pb-10 pt-24 sm:min-h-[58vh] sm:px-6 sm:pb-14 lg:px-8">
          <Link
            href={park.company === "Disney" ? "/#disney" : "/#universal"}
      ...
      <section className="sticky top-0 z-30 border-b border-navy/15 bg-white/95 backdrop-blur-md shadow-sm">
          ...
          <div className="flex items-center gap-3 py-3 overflow-x-auto scrollbar-none">
            {topData && (
              <button
                ...
                onClick={() => {
                  setActiveTab("top");
                  const el = document.getElementById("top-atracciones");
                  if (el) el.scrollIntoView({ behavior: "smooth" });
                }}
                className={cn(
                  "flex items-center gap-2 rounded-full px-5 py-2.5 text-xs sm:text-sm font-bold transition shrink-0 min-h-[40px]",
                  activeTab === "top"
                    ? "bg-navy text-offwhite shadow-sm"
                    : "bg-sand/60 text-navy/70 hover:bg-sand hover:text-navy",
                )}
              >
                <Trophy className="h-4 w-4 text-gold" />
                <span>TOP 10 & Recorrido Óptimo</span>
                <span className="rounded-full bg-gold px-1.5 py-0.2 text-[10px] font-extrabold text-navy">
                  10
                </span>
              </button>
            )}
            ...
            >
              ...
              <span>Todas las Atracciones</span>
              <span className="rounded-full bg-navy/20 px-1.5 py-0.2 text-[10px] font-bold text-navy">
      ...
      {/* TOP ATRACCIONES & RUTA RECOMENDADA SECTION */}
      {topData && (
        <TopAttractionsSection data={topData} />
      )}

      {/* FULL CATALOG SECTION & FILTERS */}
          ...
          <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-navy/60 font-medium">
                Mostrando {filtered.length} de {park.attractions.length}
              </span>
            </div>
